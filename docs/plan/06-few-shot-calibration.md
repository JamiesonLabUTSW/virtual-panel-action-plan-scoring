...
evaluation personality, and validate that calibration produces meaningful rater differentiation.

**Risk:** MEDIUM — quality depends on iterative prompt tuning **Estimated effort:** 1 day **Depends
on:** Phase 4 (working judge pipeline to test calibration effects) **Blocks:** Phase 7 (chat quality
depends on having differentiated judge outputs)

---

## Parallel Tracks

This phase is **perfectly parallelizable across 3 developers** — each rater's calibration set is
independent.
...
**3 developers:** Each person authors one rater's 5-example set independently. They can share 1-2
document excerpts for cross-calibration testing but otherwise work in isolation. After all 3 merge,
one person runs 6.4 validation.

**2 developers:** One dev takes Rater A + Rater C (most different personas — helps maintain focus).
Other dev takes Rater B. Both coordinate on shared document excerpts.
...
**Description:** Write 5 calibration examples for Rater A: an experienced academic reviewer who is
strict on structure and logical flow, lenient on style.
...
- `rationale` reflects The Professor's voice: references "structure," "logical flow,"
...
- `confidence` follows the operational scale: 0.9 for clear cases, 0.6 for borderlines, 0.3 for
  ambiguous
...
- The Professor's persona is consistent: strict on structure/logic, lenient on style across all
  examples
...
**Description:** Write 5 calibration examples for Rater B: a professional editor who is strict on
clarity and prose quality, lenient on depth.
...
- `rationale` reflects The Editor's voice: references "clarity," "phrasing," "readability," "prose
  quality," "sentence structure"
...
- Document excerpts are different from Rater A's (though they may share 1-2 for cross-calibration
  testing)
...
**Description:** Write 5 calibration examples for Rater C: an industry professional who is strict on
actionability and evidence, lenient on formality.
...
- `rationale` reflects The Practitioner's voice: references "evidence," "data," "actionability,"
  "concrete," "measurable," "support"
...
- No example penalizes informal language if the content is well-supported (that's The Editor's
  domain)

**Code quality:**

- Same formatting standards as Raters A and B
- Document excerpts include some with quantitative data and some without (to test the practitioner's
  differentiation)
...
**Description:** Run the full pipeline with all 3 calibration sets against 2-3 test documents and
verify that different raters produce meaningfully different scores and rationales.
...
**Description:** Replace the 1-2 placeholder examples per rater (from Phase 4.2) with the full
5-example sets.
